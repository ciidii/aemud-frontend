...
export class FilterPopupComponent implements OnInit {
  ...
  onRegistrationFilterChange(): void {
    // Logic handled by valueChanges subscription in initForm
  }

  applyFilters(): void {
    this.formGroup.markAllAsTouched(); // Mark all fields as touched for validation

    // Custom validation for specificRegistrationType if 'specific' is selected
    ...
    // Custom validation for cotisationStatus if cotisationMonth is selected
    ...
    // Adjust the filter object based on the conditional radio buttons
    ...
    // Adjust cotisation filters
    if (!filters.cotisationMonth) {
      // If no month is selected, remove cotisationStatus filter as well
      delete filters.cotisationStatus;
    }
    // If cotisationMonth IS selected, and cotisationStatus is '', it means 'Tous les statuts' for that month.
