...
export class SmsModelOptionDialogComponent {
    ...
    @Input() y = 0;
    // Outputs for actions
    @Output() modifyClicked = new EventEmitter<MessageTemplateModel>();
    @Output() deleteClicked = new EventEmitter<MessageTemplateModel>();
    ...
    constructor(
        ...
        private smsTemplateState: SmsTamplateStateService
    ) {
    }
    ...
    open(data: any): void {
        ...
        if (data && (data as MessageTemplateModel).modelName && (data as MessageTemplateModel).smsModel){
         // this.toaster.info("Cliquez !!")
        }
    ...
    modifyTemplate(): void {// Nous sommes en mode modification, stocker le template
        ...
        if (this.popupAddTemplateSms) {
            ...
            this.popupAddTemplateSms.openModal(this.smsTemplateState.selectedMessageTemplate?.modelName, this.smsTemplateState.selectedMessageTemplate?.smsModel); // Pré-remplir le modal
        }
        this.close()
    }
    ...
    onDeleteClick(): void {
        if (this.smsTemplateState.selectedMessageTemplate) {
            this.deleteTemplate(this.smsTemplateState.selectedMessageTemplate);
        }
    }
    ...
    deleteTemplate(templateToDelete: MessageTemplateModel): void {
            ...
            this.templateService.deleteTemplate(templateToDelete.id)
                .subscribe({
                    next: () => {
                        ...
                        if (this.smsTemplateState.selectedMessageTemplate?.id === templateToDelete.id) {
                            this.smsTemplateState.selectedMessageTemplate = null
                            // this.sms.message = '';
                        }
